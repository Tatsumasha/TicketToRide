...
class Game(object):
    ...
    def doesPlayerHaveCardsForEdge(self, player, city1, city2):
        ...
        for col in routeColors:
            if col == 'grey':
                if max([x for x in list(player.hand.values()) if x != 'wild']) \
    ...
    def placeTrains(self, player):
        ...
        if not self.doesPlayerHaveCardsForEdge(player, city1, city2):
            ...
            return "Move complete"
        if color == 'grey':
            availColor = max(x for x in list(player.hand.values()))
        else:
        ...
        if numColor not in list(range(0, availColor + 1)):
        ...
        if numColor < routeDist:  # span distance
            ...
            numWild = int(numWild)
            if numWild not in list(range(0, availWild + 1)):
                print("You do not have that many")
                return "Move complete"
        else:
            numWild = 0
    ...
    def pickTickets(self, player, minNumToSelect=1):
        ...
        tickets = {x[0]: x[1] for x in zip(list(range(len(tickets))), tickets)}
...
def playTTR():
    ...
    count = 0
    while int(numPlayers) not in list(range(1, 7)) and count < 5:
